...
def _render_pgx(gp: GeneticProfile) -> str:
    if not gp.diplotypes:
        return ""
    rows: List[str] = [
        "| Gene | Diplotype | Phenotype | Activity score | Notes |",
        "|---|---|---|---:|---|",
    ]
    for gene, dp in gp.diplotypes.items():
        ...
        caveats = "; ".join(dp.notes[:2]) if dp.notes else ""
        rows.append(f"| {gene} | {dp.diplotype} | {label} | {score} | {caveats} |")
...
def _render_caveats(gp: GeneticProfile) -> str:
    bullets: List[str] = [
        "- Consumer microarrays (23andMe / AncestryDNA) genotype only the specific SNPs on the chip. Absence of a pathogenic variant on this panel does NOT exclude rare variants of the same gene.",
        "- CYP2D6 copy number variants (\\*5 deletion, \\*xN duplications causing true ultrarapid metabolism) are NOT callable from array data. PGx calls are tentative in this class.",
        "- For comprehensive coverage consider clinical whole-genome sequencing or targeted clinical panels (Color Health, Invitae) with genetic-counselor support.",
    ]
...
Generated locally from consumer genotype data. No raw genotypes retained in this report — only derived diplotypes and phenotypes. Do not publish this file.
...
def render_markdown_report(
    ...
    """
    parts = [_render_header(gp), _render_pgx(gp), _render_actionable(gp)]
    if adjustments:
        parts.append(_section("Stack adjustments from PGx", adjustments))
